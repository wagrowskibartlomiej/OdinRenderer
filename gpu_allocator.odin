...
GPU_MIN_BYTES_FREE_REGION_THRESHOLD :: 128
...
GPU_MEMORY_HANDLE_ID_BITS ::
	(8 * size_of(GPU_MEMORY_HANDLE_BACKING_TYPE)) - GPU_MEMORY_HANDLE_INDEX_BITS
...
DIRECT_TRANSFER_SYNCED_FLAGS :: vk.MemoryPropertyFlags {
	.HOST_VISIBLE,
	.DEVICE_LOCAL,
	.HOST_COHERENT,
}

GPU_Memory_Allocator_Proc :: #type proc(
	mode: GPU_Allocator_Mode,
	request: GPU_Memory_Allocator_Request,
	handle: GPU_Memory_Handle,
...
GPU_Resource_Allocator_Proc :: #type proc(
	mode: GPU_Allocator_Mode,
	...
	/* Index of free region in which element is located, for free region managments */vk.DeviceSize,
	/* Padding */
)
...
GPU_Resource_Allocator_Request :: struct {
	...
	flags:           vk.MemoryPropertyFlags,
	extent:          vk.Extent3D, // I do not know if we can use {0,0,0} as "nil" value cause it is valid I guess
	mappable:        bool,
	allocator:       runtime.Allocator,
}
...
GPU_Taken_Region :: struct {
	using _:               GPU_Region,
	padding, back_padding: vk.DeviceSize, // `padding` is just standard padding if needed, `back_padding` is mostly used when after splitting `GPU_Region` the left size is not enough for defined threshold
	handle:                GPU_Resource_Handle,
}
...
gpu_initalize_memory_state :: proc(
	mem_state: ^GPU_Memory_State,
	mem_alloc := GPU_Memory_Allocator{gpu_default_memory_allocator, nil},
...
gpu_allocate_default_memory_blocks :: proc(
) -> (
	vram_buffers_handle, vram_images_handle: GPU_Memory_Handle,
	staging_handle: Maybe(GPU_Memory_Handle),
) {
	buff_h, buff_err := gpu_alloc(.Buffer, VRAM_FLAGS, false, GPU_DEFAULT_ALLOCATION_SIZE)
	if buff_err != nil {
		log.panic("Cannot allocate required VRAM for buffers")
	}

	img_h, img_err := gpu_alloc(.Image, VRAM_FLAGS, false, GPU_DEFAULT_ALLOCATION_SIZE)
	if img_err != nil {
		log.panic("Cannot allocate required VRAM for images")
	}

	s, s_err := gpu_alloc(.Buffer, STAGING_FLAGS, true, GPU_DEFAULT_ALLOCATION_SIZE)
	if s_err != nil {
		log.panic("Cannot allocate required staging memory")
	}

	return buff_h, img_h, s
}
...
gpu_alloc :: proc(
	...
	block := m.memory_allocator.procedure(
		.Allocate,
		request,
		{},
		m.callbacks,
		m.memory_allocator.data,
	) or_return
...
_gpu_add_block_to_state :: proc(
	...
	if mem_state._free_elem_idx == GPU_FREE_LIST_ABSENT_VALUE {
		...
		return _gpu_generate_memory_handle(
			len(mem_state.blocks) - 1,
			&mem_state._validation_id_counter,
		)
	}
	...
	case GPU_HANDLE_LIST_NEXT_ID_VALUE:
		mem_state._free_elem_idx = i32(b.handle.index)
	case:
		log.warnf(
			"Handle of memory block does not match expected values, got '%v'. Setting free elements list as not available, possible leaks.",
			b.handle,
		)
		mem_state._free_elem_idx = GPU_FREE_LIST_ABSENT_VALUE
	}
...
_gpu_remove_block_from_state :: proc(h: GPU_Memory_Handle, mem_state: ^GPU_Memory_State) {
	...
	idx := mem_state._free_elem_idx
	mem_state.blocks[h.index].handle = {
...
gpu_default_memory_allocator: GPU_Memory_Allocator_Proc : proc(
	mode: GPU_Allocator_Mode,
	r: GPU_Memory_Allocator_Request,
	handle: GPU_Memory_Handle,
	callbacks: ^vk.AllocationCallbacks,
	data: rawptr,
) -> (
	...
	#partial switch mode {
	case .Allocate:
		return _gpu_default_memory_allocator_alloc(
			c.device.handle,
			r.category,
			r.size,
			c.physical_devices.active.memory_properties,
			r.flags,
			r.map_block,
			r.allocator,
			callbacks,
...
_gpu_default_memory_allocator_alloc :: proc(
	device: vk.Device,
	category: GPU_Memory_Resource_Category,
	size: vk.DeviceSize,
	...
	case .SUCCESS:
		when CONFIG_VERBOSE_LOG do log.debugf(
			"Allocated %v %v of GPU memory with flags %v",
	...
	case:
		log.errorf(
			"GPU Memory allocation failure: %v (wanted %v bytes | requested flags %v -> found: %v)",
			result,
			size,
			flags,
			full_flags,
	...
	block.taken_regions = make(
		[dynamic]GPU_Taken_Region,
		0,
		GPU_TAKEN_REGIONS_INITAL_CAP,
		allocator,
	)
...
_gpu_default_memory_allocator_free :: proc(
	device: vk.Device,
	handle: GPU_Memory_Handle,
	callbacks: ^vk.AllocationCallbacks,
) {
...
gpu_default_static_resource_allocator: GPU_Resource_Allocator_Proc : proc(
	mode: GPU_Allocator_Mode,
	r: GPU_Resource_Allocator_Request,
	handle: GPU_Resource_Handle,
	callbacks: ^vk.AllocationCallbacks,
	data: rawptr,
) -> (
	...
	#partial switch mode {
	case .Create:
		return _gpu_default_static_resource_allocator_create(
			device,
			r.type,
			r.size,
			rend.memory.blocks[:],
			r.flags,
			r.mappable,
			rend.core.physical_devices.active.properties.limits,
			r.extent,
			callbacks,
		)
	case .Destroy:
		return {},
			_gpu_default_static_resource_allocator_destroy(
				handle,
				rend.resources.datas[handle.index],
				device,
				callbacks,
			),
			-1,
...
_gpu_default_static_resource_allocator_create :: proc(
	device: vk.Device,
	type: GPU_Resource_Type,
	size: vk.DeviceSize,
	...
	case .Vertex_Buffer, .Index_Buffer, .Staging_Buffer:
		...
		mem_index, adjust := _gpu_find_compatibile_memory_index(
			blocks,
			.Buffer,
			req,
			memory_flags,
		...
		if mem_index == -1 {
			return {}, .Memory_Not_Found, -1, 0
		}

		final_req := req
		if adjust {
			final_req.alignment, final_req.size = _gpu_readjust_requirements_for_mapping(
				final_req,
				limits,
			)
		}

		offset, padding, free_region_index := _gpu_find_offset_for_resource(
			blocks[mem_index],
			final_req.alignment,
			final_req.size,
			search_free = true,
		...
		success := _gpu_bind_buffer_memory(
			device,
			buffer,
			blocks[mem_index].memory,
		...
		return data, nil, free_region_index, padding
	case .Depth_Image:
		info := GPU_Resource_Create_Info_Presets[type]
		info.image.extent = _gpu_get_image_extent_from_swapchain()

		img, ok := _gpu_create_image(device, &info.image, callbacks)
		if !ok {
			return {}, .Creation_Failure, -1, 0
		}

		defer if err != nil {
			vk.DestroyImage(device, img, callbacks)
		}

		req := _gpu_get_image_requirements(img)
		...
		success := _gpu_bind_image_memory(
			device,
			img,
			blocks[mem_index].memory,
			vk.DeviceSize(offset),
		...
		data.type = .Depth_Image
		data.usage = usage
		data.data_size = final_req.size
		data.backing_size = final_req.size
		data.offset = vk.DeviceSize(offset)
		data.parent_idx = mem_index
		data.resource = img
	...
	case .Texture_2D_RGBA:
		if extent == {} do return {}, .Invalid_Extent, -1, 0
		info := GPU_Resource_Create_Info_Presets[.Texture_2D_RGBA]
		...
		return data, nil, free_region_index, padding
	case:
		return {}, .Not_Implemented, -1, 0
	}
}

_gpu_default_static_resource_allocator_destroy :: proc(
	handle: GPU_Resource_Handle,
	...
	case .Vertex_Buffer, .Staging_Buffer, .Index_Buffer:
		vk.DestroyBuffer(device, gpu_get_resource_from_handle(handle).(vk.Buffer), callbacks)
		return nil
	case .Depth_Image, .Texture_2D_RGBA:
		vk.DestroyImage(device, gpu_get_resource_from_handle(handle).(vk.Image), callbacks)
		return nil
	case:
		return .Not_Implemented
	}
...
_gpu_find_compatibile_memory_index :: proc(
...
_gpu_get_resource_category_from_type :: proc(
	type: GPU_Resource_Type,
) -> GPU_Memory_Resource_Category {
	switch type {
	case .Custom_Buffer, .Vertex_Buffer, .Index_Buffer, .Staging_Buffer:
		return .Buffer
	case .Custom_Image, .Depth_Image, .Texture_2D_RGBA:
		return .Image
	}
