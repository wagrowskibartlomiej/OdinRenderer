...
engine_init :: proc "contextless" (state: ^Engine_Global_State, procs := Engine_State_Create_Procs{}) -> (ctx: runtime.Context) {
	...
	// assets system initalization

	return state.app_context.ctx
}

engine_cleanup :: proc(state: ^Engine_Global_State, procs := Engine_State_Cleanup_Procs{}) {
	// cleanup assets system
	cleanup_engine_configuration()
	cleanup_engine_state(state, procs)
}

engine_renderer_init :: proc(state: ^Engine_Global_State, platform_context: rawptr) -> (success: bool) {
	success = create_window(&state.window, platform_context)
	success or_return
...
engine_renderer_cleanup :: proc(state: ^Engine_Global_State, platform_context: rawptr) {
	...
	cleanup_vulkan(&state.renderer)
	cleanup_window(&state.window, platform_context)
}
